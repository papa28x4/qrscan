...
<div class="content-wrapper lime-body">  
    <form class="bg-white" action="{{route('admins.update', $admin->slug)}}" method="POST">
        ...
        <div class="row">
            ...
            <div class="col-md-4 border-right">
                <div class="d-flex flex-column align-items-center text-center p-3 py-5">
                    {{-- <img class="rounded-circle mt-5" src="{{$admin->avatar}}" width="90"> --}}
                    ...
                    {{-- <div class="d-flex flex-column mt-2">
                        <span class="font-weight-bold mt-1">{{$admin->fullname()}}</span>
                        <span class="text-black-50 mt-1">{{$admin->email}}</span>
                        <span class="mt-1">{{$admin->country ?? '' }}</span>
                    </div> --}}
            ...
            <div class="col-md-8">
                <div class="p-3 py-5">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <h6 class="text-right">Edit Admin</h6>
                    </div>
                    <div class="row mt-2">
                        <div class="col-md-6">
                            <input name="name" type="text" class="form-control" placeholder="First name" value="{{old('name') ?? $admin->name}}">
                            @error('name')
                            <small id="nameHelp" class="form-text text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            <input name="last_name" type="text" class="form-control" value="{{old('last_name') ?? $admin->last_name}}" placeholder="Last name">
                            @error('last_name')
                    ...
                    <div class="row mt-3">
                        <div class="col-md-6">
                            <input name="email" type="text" class="form-control" placeholder="Email" value="{{old('email') ?? $admin->email}}">
                            @error('email')
                    ...
                    <div class="row mt-3">
                        <div class="col-md-6">
                            <input name="phone" type="text" class="form-control" value="{{old('phone') ?? $admin->phone}}" placeholder="Phone number">
                            @error('phone')
                    ...
                    <div class="mt-5 text-right"><button class="btn btn-success profile-button" type="submit">Update Admin</button></div>
                </div>
            </div> 
        </div>
    </form>  
