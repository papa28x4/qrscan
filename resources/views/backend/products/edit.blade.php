                    ...
                    <div class="row">
                      <div class="col-lg-11 col-md-12 col-sm-12 mx-auto">
                        <div class="tm-bg-primary-dark tm-block tm-block-h-auto">
                          <div class="row">
                            <div class="col-12">
                              <h2 class="tm-block-title d-inline-block px-1 mb-4">Edit Product</h2>
                            </div>
                          </div>
                          <form action="{{route('products.update', $product->slug)}}" method="post" class="tm-edit-product-form" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="col-xl-6 col-lg-6 col-md-12 px-4 order-2 order-lg-1">
                                    ...
                                    <div class="form-group mb-3">
                                      ...
                                      <select
                                        ...
                                        name="category"
                                      >
                                        {{-- <option disabled value="select" selected>Select category</option> --}}
                                        @foreach ($categories as $category)
                                          <option @if(old('category') == $category->id) selected  @elseif($product->category->id === $category->id) selected  @endif value="{{$category->id}}" >{{$category->name}}</option>
                                        @endforeach
                                      </select>
                                      @error('category')
                                      ...
                                      <div class="form-group mb-3 col-xs-12 col-sm-6">
                                        ...
                                        >
                                          ...
                                          @foreach ($brands as $brand)
                                            <option @if(old('brand') == $brand->id || $product->brand_id == $brand->id) selected
...
@push('child-scripts')
    ...
    <script>
        $(document).ready(function() {
          let images = `<?php echo json_encode($product->images[0]['temp_url']) ?>`
          let preloaded = images ? JSON.parse(images) : []
